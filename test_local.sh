...
run_ok() {
    local desc="$1"; shift
    if "$@"; then pass "$desc"; else fail "$desc"; exit 1; fi
}
...
# ─── Unit tests ─────────────────────────────────────────
section "Unit tests"
./cmake-build-debug/test_format
./cmake-build-debug/test_backup
...
backup_it() {
    ./cmake-build-debug/s3backup --local -b dummy "$_IMG" "$_SRC" >/dev/null 2>&1
}
...
run_ok  "fsck passes"                   ./cmake-build-debug/s3check fsck --local "$_IMG"
run_ok  "diff matches original"         ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"

section "Test 2: Content modification"
fresh t2
echo "original" > "$_SRC/data.txt"
backup_it
run_ok   "diff matches before change"   ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
echo "modified" > "$_SRC/data.txt"
run_fail "diff detects content change"   ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"

section "Test 3: New file"
fresh t3
echo "keep" > "$_SRC/keep.txt"
backup_it
run_ok   "diff matches before add"      ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
echo "new" > "$_SRC/added.txt"
run_fail "diff detects new file"         ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
echo "b" > "$_SRC/b.txt"
backup_it
run_ok   "diff matches before delete"   ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
rm "$_SRC/b.txt"
run_fail "diff detects deleted file"     ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
run_fail "diff detects symlink change"   ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"

section "Test 6: Empty directory"
fresh t6
backup_it
run_ok  "backup handles empty dir"      test -f "$_IMG"
run_ok  "fsck passes on empty"          ./cmake-build-debug/s3check fsck --local "$_IMG"
run_ok  "diff matches empty dir"        ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
./cmake-build-debug/s3backup --local -b dummy -e ".ssh" "$_IMG" "$_SRC" >/dev/null 2>&1
run_ok  "fsck passes with exclusions"   ./cmake-build-debug/s3check fsck --local "$_IMG"
...
run_ok  "excluded .ssh absent from backup" ./cmake-build-debug/s3check diff --local "$_IMG" "$EXCL_VERIFY"
...
run_ok  "fsck passes on 1000 files"     ./cmake-build-debug/s3check fsck --local "$_IMG"
run_ok  "diff matches 1000 files"       ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
run_ok  "fsck passes sector boundaries" ./cmake-build-debug/s3check fsck --local "$_IMG"
run_ok  "diff matches sector boundaries" ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
run_ok  "fsck passes with absolute symlink" ./cmake-build-debug/s3check fsck --local "$_IMG"
run_ok  "diff matches absolute symlink"  ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
./cmake-build-debug/s3backup --local -b dummy "$FULL_IMG" "$_SRC" >/dev/null 2>&1
run_ok  "full backup created"           test -f "$FULL_IMG"
run_ok  "full fsck passes"              ./cmake-build-debug/s3check fsck --local "$FULL_IMG"
run_ok  "full diff matches"             ./cmake-build-debug/s3check diff --local "$FULL_IMG" "$_SRC"
...
./cmake-build-debug/s3backup --local -b dummy -i "$FULL_IMG" "$INCR_IMG" "$_SRC" >/dev/null 2>&1
run_ok  "incremental backup created"    test -f "$INCR_IMG"
run_ok  "incremental fsck passes"       ./cmake-build-debug/s3check fsck --local "$INCR_IMG"
...
run_fail "fsck rejects truncated image"  ./cmake-build-debug/s3check fsck --local "$TRUNC_IMG"

# Zero-byte image
ZERO_IMG="$TEST_ROOT/t12_zero.img"
: > "$ZERO_IMG"
run_fail "fsck rejects empty file"       ./cmake-build-debug/s3check fsck --local "$ZERO_IMG"

# Garbage image
GARBAGE_IMG="$TEST_ROOT/t12_garbage.img"
head -c 4096 /dev/urandom > "$GARBAGE_IMG"
run_fail "fsck rejects garbage file"     ./cmake-build-debug/s3check fsck --local "$GARBAGE_IMG"
...
./cmake-build-debug/s3check fsck --local "$FLIP_IMG" >/dev/null 2>&1 || true
...
run_ok  "fsck passes on 5MB"            ./cmake-build-debug/s3check fsck --local "$_IMG"
run_ok  "diff matches 5MB"              ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
...
run_ok  "fsck passes on 500MB"          ./cmake-build-debug/s3check fsck --local "$_IMG"
echo "  Running diff..."
T0=$(date +%s)
run_ok  "diff matches 500MB"            ./cmake-build-debug/s3check diff --local "$_IMG" "$_SRC"
T1=$(date +%s)
echo "  Diff: $((T1-T0))s"
...
for RUN in 1 2 3; do
    ...
    run_ok  "run ${RUN}: backup + fsck + diff" \
        sh -c "./cmake-build-debug/s3check fsck --local '$_IMG' >/dev/null 2>&1 && ./cmake-build-debug/s3check diff --local '$_IMG' '$_SRC' >/dev/null 2>&1"
...
./cmake-build-debug/s3check fsck --local "$TEST_ROOT/t1.img" 2>&1 | sed 's/^/    /'
...
run_ok  "s3 backup 5MB" \
    ./cmake-build-debug/s3backup --bucket "$S3_BUCKET" --protocol http backups/full-001 "$S3_SRC"

run_ok  "s3 fsck" \
    ./cmake-build-debug/s3check fsck --http "$S3_BUCKET/backups/full-001"

run_ok  "s3 diff matches" \
    ./cmake-build-debug/s3check diff --http "$S3_BUCKET/backups/full-001" "$S3_SRC"
...
run_ok  "s3 backup sector boundaries" \
    ./cmake-build-debug/s3backup --bucket "$S3_BUCKET" --protocol http backups/sector-001 "$S3_SECT"
run_ok  "s3 fsck sector boundaries" \
    ./cmake-build-debug/s3check fsck --http "$S3_BUCKET/backups/sector-001"
run_ok  "s3 diff sector boundaries" \
    ./cmake-build-debug/s3check diff --http "$S3_BUCKET/backups/sector-001" "$S3_SECT"
...
run_ok  "s3 backup 500MB" \
    ./cmake-build-debug/s3backup --bucket "$S3_BUCKET" --protocol http backups/scale-001 "$S3_SCALE"
T1=$(date +%s)
echo "  Upload: $((T1-T0))s"

run_ok  "s3 fsck 500MB" \
    ./cmake-build-debug/s3check fsck --http "$S3_BUCKET/backups/scale-001"

echo "  Verifying content (streaming SHA-256 over S3)..."
T0=$(date +%s)
run_ok  "s3 diff 500MB" \
    ./cmake-build-debug/s3check diff --http "$S3_BUCKET/backups/scale-001" "$S3_SCALE"
T1=$(date +%s)
echo "  Verify: $((T1-T0))s"
